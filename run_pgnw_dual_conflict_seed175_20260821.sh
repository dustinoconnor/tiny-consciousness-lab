...
run_resource="outputs/resource_memory_pgnw_dual_conflict_seed175_20260821.json"
typed_memory="outputs/typed_interaction_pgnw_dual_conflict_seed175_20260821.json"
metabolic_memory="outputs/typed_metabolic_role_pgnw_dual_conflict_seed175_20260821.json"
run_log="outputs/unity_shadow/pgnw_dual_conflict_seed175_20260821.jsonl"

for required in "$scene_path" "$source_resource" "$source_metabolic"; do
  if [[ ! -f "$required" ]]; then
...
if [[ -e "$run_resource" || -e "$typed_memory" || -e "$metabolic_memory" || -e "$run_log" ]]; then
  print -u2 "Refusing to append: seed-175 dual-conflict output exists."
  exit 2
fi
