...
run_memory="outputs/resource_memory_typed_metabolic_seed171_20260820.json"
typed_memory="outputs/typed_interaction_typed_metabolic_seed171_20260820.json"
metabolic_memory="outputs/typed_metabolic_role_seed171_20260820.json"
run_log="outputs/unity_shadow/typed_metabolic_role_seed171_20260820.jsonl"

if [[ ! -f "$source_memory" ]]; then
  print -u2 "Missing frozen seed-163 acquisition memory."
  exit 2
fi
if [[ -e "$run_memory" || -e "$typed_memory" || -e "$metabolic_memory" || -e "$run_log" ]]; then
  print -u2 "Refusing to append: seed-171 metabolic-role output exists."
  exit 2
fi
...
exec caffeinate -dimsu python3 embodied_unity_loop.py \
  ...
  --pgnw-multi-hypothesis-arbitration bounded_verified \
