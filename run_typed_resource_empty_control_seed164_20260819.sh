...
run_memory="outputs/resource_memory_recall_seed164_empty_control_20260819.json"
typed_memory="outputs/typed_interaction_recall_seed164_empty_control_20260819.json"
run_log="outputs/unity_shadow/typed_resource_recall_seed164_empty_control_20260819.jsonl"

if [[ -e "$run_memory" || -e "$typed_memory" || -e "$run_log" ]]; then
  print -u2 "Refusing to append: seed-164 empty-memory control already exists."
  exit 2
fi

# Identical to the persistent-recall treatment except that this resource-memory
# path does not exist at startup. Live pickups may be encoded, but the controller
# begins with no remembered yellow coordinate.
