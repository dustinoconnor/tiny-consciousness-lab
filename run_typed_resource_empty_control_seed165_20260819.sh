...
run_memory="outputs/resource_memory_recall_seed165_empty_control_20260819.json"
typed_memory="outputs/typed_interaction_recall_seed165_empty_control_20260819.json"
run_log="outputs/unity_shadow/typed_resource_recall_seed165_empty_control_20260819.jsonl"

if [[ -e "$run_memory" || -e "$typed_memory" || -e "$run_log" ]]; then
  print -u2 "Refusing to append: seed-165 empty-memory control already exists."
  exit 2
fi

# Counterbalanced second pair: control runs before treatment from a new red.
