...
run_memory="outputs/resource_memory_recall_seed164_20260819.json"
typed_memory="outputs/typed_interaction_recall_seed164_20260819.json"
run_log="outputs/unity_shadow/typed_resource_recall_seed164_20260819.jsonl"
...
if [[ -e "$run_memory" || -e "$typed_memory" || -e "$run_log" ]]; then
  print -u2 "Refusing to append: seed-164 recall output already exists."
  exit 2
fi

# Preserve the acquisition artifact and let this run mutate only its copy.
cp "$source_memory" "$run_memory"

exec caffeinate -dimsu python embodied_unity_loop.py \
  --duration 300 \
  --seed 164 \
  --diagnostic-teleport 72.43 -82.33 \
