...
run_memory="outputs/resource_memory_recall_seed164_retry_20260819.json"
typed_memory="outputs/typed_interaction_recall_seed164_retry_20260819.json"
run_log="outputs/unity_shadow/typed_resource_recall_seed164_retry_20260819.jsonl"
...
if [[ -e "$run_memory" || -e "$typed_memory" || -e "$run_log" ]]; then
  print -u2 "Refusing to append: seed-164 retry output already exists."
  exit 2
fi

cp "$source_memory" "$run_memory"

# Start 7.93 units west of the remembered red so telemetry establishes a
# zero-red baseline before direct perception and physical pickup.
